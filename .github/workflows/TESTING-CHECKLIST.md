...
- [x] memory-sanitizer - Documented (skip note about instrumented libc++)
...
4. **MemorySanitizer Job**
   - Status: Documented but not active (requires complex setup)
   - Future: Can be enabled with proper instrumented libc++ setup
...
- MSAN requires instrumented C++ standard library (not enabled)
...
4. Consider periodic runs with MSAN using custom Docker image
